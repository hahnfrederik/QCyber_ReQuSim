...
def twolink_compatible_world():
    ...
    source_A = SchedulingSource(world=world, position=station_central.position,
                                target_stations=[station_A, station_central], time_distribution=lambda source:  2,
                                state_generation=lambda source:  mat.H(mat.phiplus) @ mat.phiplus)
    source_B = SchedulingSource(world=world, position=station_central.position,
                                target_stations=[station_central, station_B], time_distribution=lambda source: 2,
                                state_generation=lambda source: mat.H(mat.phiplus) @ mat.phiplus)
    return world
...
def test_setup_takes_precedence(twolink_compatible_world, twolink_compatible_world2):
    ...
    comm_speed2 = 2e8
    with pytest.warns(FutureWarning):
        p = EmptyTwoLinkProtocol(world=twolink_compatible_world, communication_speed=comm_speed1)
